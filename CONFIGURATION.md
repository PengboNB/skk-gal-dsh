...
- GAL 视窗：一键启用或关闭插件主题。默认启用；关闭后会停止全局美化、剧场自动跳转和回复风格注入，并恢复 DSH 原始外观。
- 玩家名称：显示在底部用户专属对话框中。
...
GAL 视窗快捷设置顶部提供“GAL 视窗总开关”。此开关与剧场内设置共用同一份浏览器本地配置：安装后默认开启；如果用户手动关闭，插件会移除侧边栏、顶部导航、新对话页和会话页的主题样式，尽量回到 DSH 原始深色外观。
...
侧边栏与主对话区域之间会显示一条细窄的冰蓝拖拽分隔线。鼠标悬停后按住拖动即可调整侧边栏宽度；插件会把宽度保存在当前浏览器，最大宽度限制为当前窗口宽度的三分之一。点击 DSH 自带的收起侧边栏按钮后，侧边栏主题背景会同步压缩到窄栏，不会再保留展开状态的背景宽度。
