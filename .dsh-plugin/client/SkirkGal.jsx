...
import { nodesToLines, partialText } from './transcript.mjs'

const SPEED = { slow: 48, normal: 25, fast: 10 }
        ...
        <>
          ...
          <label className="skk-setting">打字速度
            <select value={settings.speed} onChange={e => setSettings(s => ({ ...s, speed: e.target.value }))}>
              <option value="slow">慢</option><option value="normal">正常</option><option value="fast">快</option>
            </select>
          </label>
  ...
  return (
    <main ref={rootRef} className="skk-root" data-motion={settings.motion ? 'on' : 'off'}>
      ...
      <section className="skk-response-zone" aria-label="丝柯克回复">
        <header className="skk-response-head"><span>丝柯克 · 回响记录</span><span className="skk-response-tools"><button className="skk-action" type="button" onClick={skip}>显示全文</button><button className="skk-action" type="button" onClick={() => setPanel('history')}>对话记录</button></span></header>
